...
\textbf{Schritt 3: Beide Eigenschaften einsetzen.}

\textbf{(a) Linke Seite umschreiben} mit Eigenschaft (a):
\[
\det(A^\top) = \det(A).
\]

\textbf{(b) Rechte Seite umschreiben} mit Eigenschaft (b):
\[
\det(-A) = (-1)^n \det(A).
\]

\textbf{(c) Gleichsetzen.} Aus Schritt 2 wissen wir, dass die linke und rechte Seite gleich sind, also:
\[
\det(A) = (-1)^n \det(A).
\]
