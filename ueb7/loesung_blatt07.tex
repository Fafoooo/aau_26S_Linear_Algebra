...
\textbf{Warum streicht man Zeile $i$ \emph{und} Spalte $j$ weg?}\\
In der Determinante muss man aus jeder Zeile und aus jeder Spalte \textbf{genau einen} Eintrag w\"ahlen (so ist die Determinante \"uber Permutationen definiert). Wenn ich also den Eintrag $a_{ij}$ als ``Beitrag'' aus Zeile $i$ ausw\"ahle, dann ist Spalte $j$ damit ebenfalls schon ``verbraucht'' -- in dieser Spalte darf kein weiterer Eintrag mehr genommen werden.

Die anderen Eintr\"age, die ich brauche, kommen also \textbf{nicht aus Zeile $i$} und \textbf{nicht aus Spalte $j$}. Genau diese ``\"ubrige'' kleinere Matrix erh\"alt man, wenn man Zeile $i$ und Spalte $j$ einfach durchstreicht. Davon nehme ich die Determinante (den Minor $M_{ij}$).
...
In Spalte 3 sind vier von f\"unf Eintr\"agen null. Beim Aufsummieren \"uber die Spalte werden alle vier ihrer Beitr\"age $0 \cdot \dots = 0$. \"Ubrig bleibt nur der Eintrag $13$ in Zeile 3.

\textbf{Was streichen wir weg?} Da der einzige nichttriviale Beitrag aus Position $(3, 3)$ kommt: \textbf{Zeile 3 und Spalte 3} der $5\times 5$-Matrix wegstreichen. Was \"ubrig bleibt, ist eine $4 \times 4$-Matrix; nennen wir sie $M$.
...
Einziger nichttrivialer Beitrag: Eintrag $(1, 4) = 2$. \textbf{Zeile 1 und Spalte 4} von $M$ wegstreichen $\Rightarrow$ $3 \times 3$-Matrix $N$.
...
Einziger nichttrivialer Beitrag: Eintrag $(2, 1) = 1$. \textbf{Zeile 2 und Spalte 1} von $N$ wegstreichen $\Rightarrow$ $2 \times 2$-Matrix.
...
\textbf{Warum erste Zeile?} Die Matrix hat keine Nullen, also ist es egal, wo ich entwickle.
Da der Parameter $a$ in Position $(1,1)$ steht, ist es bequem, nach der ersten Zeile zu entwickeln --
so taucht $a$ direkt im Ergebnis auf, ohne erst durch andere Minoren zu wandern.

\textbf{Erste Zeile ablesen.} Die Eintr\"age sind:
\[
a_{11} = a, \quad a_{12} = 1, \quad a_{13} = 2, \quad a_{14} = 3.
\]

\textbf{Vorzeichen f\"ur die erste Zeile} (aus $(-1)^{1+j}$): $+, -, +, -$.

\textbf{Allgemeine Laplace-Formel} f\"ur Entwicklung nach Zeile $1$:
\[
\det A = \sum_{j=1}^{4} (-1)^{1+j} \cdot a_{1j} \cdot M_{1j}
= (+1)\,a_{11}\,M_{11} \;+\; (-1)\,a_{12}\,M_{12} \;+\; (+1)\,a_{13}\,M_{13} \;+\; (-1)\,a_{14}\,M_{14}.
\]

\textbf{Eintr\"age einsetzen.}
\[
\det A = (+1)\,a\,M_{11} \;+\; (-1)\,(1)\,M_{12} \;+\; (+1)\,(2)\,M_{13} \;+\; (-1)\,(3)\,M_{14},
\]
also kompakt:
\[
...
Die vier $3\times 3$-Minoren $M_{1j}$ entstehen, indem man jeweils \textbf{Zeile $1$ und Spalte $j$} wegstreicht.
