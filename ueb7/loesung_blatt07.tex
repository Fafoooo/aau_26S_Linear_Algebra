...
\begin{itemize}[leftmargin=*, nosep]
    ...
    \item \textbf{Laplace-Entwicklung} -- Entwicklung nach einer Zeile oder Spalte:\\ $\det(A) = \sum_{j} (-1)^{i+j} \, a_{ij} \, M_{ij}$, wobei $M_{ij}$ die Minor-Determinante ist (Matrix ohne Zeile $i$ und Spalte $j$).
    \item \textbf{Trick}: Zeilen/Spalten mit vielen Nullen suchen $\Rightarrow$ Laplace-Entwicklung wird einfach.
\end{itemize}
\end{begriffe}
...
Zeile 3 hat zwei Nullen $\Rightarrow$ nur ein Term \"uberlebt:
\[
= 1 \cdot (-1)^{3+3} \cdot \det \begin{pmatrix} 234 & 235 \\ 1 & 1 \end{pmatrix}
= 234 \cdot 1 - 235 \cdot 1 = -1.
\]
