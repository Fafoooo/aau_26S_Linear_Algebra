...
\textbf{Strategie:} Die Aufgabenstellung verlangt, ohne neue Rechnung allein aus (a) und (b) zu schlie\ss en. Wir verwenden die Charakterisierungen aus dem Begriffsblock.

\medskip

\textbf{Injektiv?} ``Injektiv'' bedeutet anschaulich: \emph{verschiedene Inputs liefern verschiedene Outputs}. Das ist genau dann der Fall, wenn $\text{Ker}(T) = \{0\}$, also nur der Nullvektor auf $0$ abgebildet wird.

Aus (a): $\dim \text{Ker}(T) = 3 \neq 0 \;\Rightarrow\; \text{Ker}(T)$ enth\"alt von $0$ verschiedene Vektoren \\
$\Rightarrow\;$ $T$ ist \textbf{nicht injektiv}.

\medskip

\textbf{Surjektiv?} ``Surjektiv'' bedeutet anschaulich: \emph{jeder Vektor im Zielraum wird getroffen}. Das ist genau dann der Fall, wenn $\text{Im}(T)$ den ganzen Zielraum ausf\"ullt, also $\dim \text{Im}(T) = \dim W$.

Aus (b): $\dim \text{Im}(T) = 2$, aber $\dim \mathbb{R}^3 = 3 \;\Rightarrow\; \text{Im}(T)$ ist ein echter Unterraum von $\mathbb{R}^3$ \\
$\Rightarrow\;$ $T$ ist \textbf{nicht surjektiv}.

\medskip

\textbf{Bijektiv?} ``Bijektiv'' bedeutet \emph{injektiv und surjektiv zugleich}. Da bereits beides einzeln scheitert, ist $T$ erst recht \textbf{nicht bijektiv}.
...
\textbf{Definition.} $T$ hei\ss t \emph{invertierbar}, wenn es eine lineare Abbildung $T^{-1}$ gibt, die $T$ r\"uckg\"angig macht: $T^{-1}(T(v)) = v$ f\"ur jedes $v$.

\textbf{Charakterisierung:} $T$ ist invertierbar $\iff$ $T$ ist bijektiv.

\medskip

\textbf{Anwendung auf unseren Fall.} Aus (d): $T$ ist nicht bijektiv $\Rightarrow$ $T$ ist \textbf{nicht invertierbar}.

\medskip

\textbf{Anschaulich: warum?} Es gibt zwei intuitive Gr\"unde:
\begin{itemize}[leftmargin=*, nosep]
    \item \emph{Informationsverlust (aus Nicht-Injektivit\"at):} Mehrere verschiedene $v$ werden auf denselben Output abgebildet. Aus dem Output l\"asst sich nicht eindeutig rekonstruieren, welches $v$ es war.
    \item \emph{Nicht-erreichbare Outputs (aus Nicht-Surjektivit\"at):} Es gibt Vektoren in $\mathbb{R}^3$, die gar nicht im Bild liegen. F\"ur diese w\"are $T^{-1}$ nicht definiert.
\end{itemize}

\textbf{Dimensionsargument (alternativer Beweis).} F\"ur jede invertierbare Abbildung $T \colon V \to W$ muss $\dim V = \dim W$ gelten. Hier ist $\dim \mathbb{R}^5 = 5 \neq 3 = \dim \mathbb{R}^3$ -- $T$ kann allein deshalb niemals invertierbar sein.
