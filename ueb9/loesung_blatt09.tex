...
Erweiterte Matrix mit rechter Seite $(1, 2, 5)^\top$, dieselben Zeilenoperationen wie in Schritt 2:
\[
...
Keine Widerspruchszeile $\Rightarrow$ konsistent.

\textbf{Partikul\"are L\"osung} (setze $c = d = e = 0$): $a = 2$, $b = 1$.
\[
v_0 = (2, 1, 0, 0, 0).
\]
\textbf{Probe:} $T(2,1,0,0,0) = (1 + 0 + 0 + 0,\; 2 + 0 + 0 + 0,\; 8 - 3 + 0 - 0) = (1, 2, 5).$ \checkmark

\textbf{Allgemeine L\"osung} = partikul\"are L\"osung + Kern:
\[
\fbox{\parbox{0.93\textwidth}{\centering
$v = (2, 1, 0, 0, 0)
+ s(-1,-1,1,0,0)
...
\textbf{Aus (a):} $\dim \text{Ker}(T) = 3 \neq 0$ $\Rightarrow$ $T$ ist \textbf{nicht injektiv}.

\textbf{Aus (b):} $\dim \text{Im}(T) = 2 \neq 3 = \dim \mathbb{R}^3$ $\Rightarrow$ $T$ ist \textbf{nicht surjektiv}.

Da bijektiv $= $ injektiv \emph{und} surjektiv, ist $T$ auch \textbf{nicht bijektiv}.

%% --- 35 (e) ---
\subsubsection*{(e) Invertierbar?}

$T$ ist invertierbar $\iff$ $T$ bijektiv. Aus (d): nicht bijektiv $\Rightarrow$ \textbf{$T$ ist nicht invertierbar}.

(Alternative Begr\"undung: F\"ur $T \colon \mathbb{R}^5 \to \mathbb{R}^3$ k\"onnen Definitions- und Wertebereich gar nicht gleichdimensional sein, also kann $T$ niemals bijektiv sein.)
