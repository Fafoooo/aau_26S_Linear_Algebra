...
\textbf{Step 1: $\lambda = 0$.} Row $1 =$ row $3$, and two equal rows give $\det A = 0$. So $A$ is not invertible, i.e.\ there is $v \neq 0$ with $Av = 0 = 0\cdot v$ -- $\lambda = 0$ is an eigenvalue, and the eigenspace is $\ker A$. The rows of $Av = 0$ as equations:
\[
\text{Row 2}\ (0,6,0): \ 6y = 0 \Rightarrow y = 0; \qquad \text{Row 1}\ (1,0,5): \ x + 5z = 0 \Rightarrow x = -5z.
\]
$z$ is free; with $z = 1$:
\[
...
\textbf{Step 2: $\lambda = 6$.} The middle column is $(0,6,0)$, i.e.\ $A e_2 = 6 e_2$ -- so $6$ is an eigenvalue with eigenvector $e_2$. The whole eigenspace is $\ker(A - 6I)$:
\[
...
\]
Rows as equations: $(-5,0,5) \Rightarrow -5x + 5z = 0 \Rightarrow x = z$; $(1,0,-1) \Rightarrow x - z = 0$ (the same). No condition on $y$, so $y$ and $z$ are free. With $(y,z) = (1,0)$ and $(0,1)$:
\[
E_6 = \operatorname{span}\{(0,1,0),\, (1,0,1)\}, \qquad \text{GM}(6) = 2.
\]

\textbf{Step 3: Multiplicities.} A $3\times3$ matrix has $3$ eigenvalues in total (with multiplicity), so the AMs sum to $3$. Since GM $\le$ AM, we have $\text{AM}(6) \ge \text{GM}(6) = 2$, and $0$ is an eigenvalue, so $\text{AM}(0) \ge 1$. Already $2 + 1 = 3$ -- everything is used up: $\text{AM}(6) = 2$, $\text{AM}(0) = 1$.
