...
\textbf{Schritt 1: Eigenwert $\lambda = 0$} (weil Zeile $1 =$ Zeile $3$). Zwei gleiche Zeilen $\Rightarrow \det A = 0$, also ist $A$ nicht invertierbar. Das bedeutet genau: es gibt ein $v \neq 0$ mit $Av = 0$ -- und wegen $0 = 0 \cdot v$ ist das ein Eigenvektor zum Eigenwert $\mathbf{0}$.

Die Eigenvektoren sind die L\"osungen von $Av = 0$. Die Zeilen von $A$ als Gleichungen:
\[
\text{Zeile 2:}\ \ 6y = 0 \Rightarrow y = 0; \qquad \text{Zeile 1:}\ \ x + 5z = 0 \Rightarrow x = -5z.
\]
$z$ ist frei. Mit $z = 1$:\quad $E_0 = \operatorname{span}\{(-5, 0, 1)\}$, \ also $\text{GM}(0) = 1$.

\textbf{Schritt 2: Eigenwert $\lambda = 6$} (aus der mittleren Spalte). Spalte $2$ von $A$ ist $(0,6,0) = 6\cdot(0,1,0)$, also $A\,e_2 = 6\,e_2$ -- das ist genau ein Eigenvektor zum Eigenwert $\mathbf{6}$.

Alle Eigenvektoren zu $6$ sind die L\"osungen von $(A - 6I)v = 0$:
\[
...
\]
Zeile $1$: $-5x + 5z = 0 \Rightarrow x = z$ (Zeile $3$ sagt dasselbe). Auf $y$ keine Bedingung, also sind $y$ und $z$ frei. Mit $(y,z) = (1,0)$ und $(0,1)$:
\[
E_6 = \operatorname{span}\{(0,1,0),\, (1,0,1)\}, \qquad \text{GM}(6) = 2.
\]

\textbf{Schritt 3: Vielfachheiten und Fazit.} Zu $\lambda = 6$ haben wir $\mathbf{2}$ unabh\"angige Eigenvektoren, zu $\lambda = 0$ einen -- zusammen $3$, genau so viele, wie $A$ gro\ss{} ist ($3\times3$). Damit ist die Liste der Eigenwerte voll: $6$ z\"ahlt \textbf{doppelt} (AM $2$), $0$ \textbf{einfach} (AM $1$). Und $3$ unabh\"angige Eigenvektoren bilden eine ganze Basis $\Rightarrow$ $A$ ist diagonalisierbar.
\erglerk\ \textcolor{gray}{\small Probe mit der Spur: $0 + 6 + 6 = 12 = 1+6+5$ \checkmark.}
