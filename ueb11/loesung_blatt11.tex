...
\textbf{Schritt 2: Mit Spaltenoperationen auf Dreiecksform.} Ziel: Wir wollen oben rechts Nullen erzeugen. Denn bei einer \emph{Dreiecksmatrix} (oben rechts nur Nullen) ist die Determinante einfach das Produkt der Diagonale -- nichts mehr zu rechnen. Die erste Zeile ist $(1,1,1)$; ziehen wir Spalte $1$ von Spalte $2$ und von Spalte $3$ ab, entstehen dort oben genau die Nullen.

\emph{$C_2 \to C_2 - C_1$} (Spalte 1 von Spalte 2 abziehen, Eintrag f\"ur Eintrag): \; oben $1-1 = 0$, \; Mitte $(5-t)-2 = 3-t$, \; unten $-1-(-1) = 0$.

\emph{$C_3 \to C_3 - C_1$}: \; oben $1-1 = 0$, \; Mitte $2-2 = 0$, \; unten $(2-t)-(-1) = 3-t$.
\[
(5-t)\,\det\!\begin{pmatrix} 1 & 1 & 1 \\ 2 & 5-t & 2 \\ -1 & -1 & 2-t \end{pmatrix}
= (5-t)\,\det\!\begin{pmatrix} 1 & 0 & 0 \\ 2 & 3-t & 0 \\ -1 & 0 & 3-t \end{pmatrix}.
\]
(Solche Spaltenoperationen ``Spalte $-$ andere Spalte'' \"andern die Determinante nicht, genau wie bei Zeilen.) Die rechte Matrix ist Dreiecksform, also Determinante $=$ Produkt der Diagonale:
\[
...
\]
\textbf{Von der Stufenform zum Eigenvektor.} Die Stufenform als Gleichungssystem gelesen (Variablen $x, y, z$):
\[
\text{Zeile 1:}\;\; -x + y + z = 0, \qquad \text{Zeile 2:}\;\; 2y + 4z = 0, \qquad \text{Zeile 3:}\;\; 0 = 0.
\]
Zeile 3 ist $0 = 0$, sagt also nichts. Es bleiben $2$ Gleichungen f\"ur $3$ Unbekannte $\Rightarrow$ eine Variable ist \textbf{frei} w\"ahlbar. Welche? Die Pivots (rot) stehen in den Spalten von $x$ und $y$; die $z$-Spalte hat \emph{keinen} Pivot, also ist \textbf{$z$ die freie Variable}.

Jetzt von unten nach oben aufl\"osen (Rest in Abh\"angigkeit von $z$ ausdr\"ucken):
\begin{itemize}[leftmargin=*, nosep]
  \item Zeile 2: $2y + 4z = 0 \Rightarrow y = -2z$.
  \item Zeile 1: $-x + y + z = 0 \Rightarrow x = y + z = -2z + z = -z$.
\end{itemize}
Jede L\"osung hat also die Form $(x, y, z) = (-z,\, -2z,\, z) = z \cdot (-1, -2, 1)$. Setzt man $z = 1$, bekommt man einen konkreten Vektor; jedes andere $z$ liefert nur ein Vielfaches davon (dieselbe Richtung). \textbf{Eine} freie Variable $\Rightarrow$ \textbf{eine} Basisrichtung, also ist GM $= 1$:
\[
...
\]
\textbf{Von der Stufenform zum Eigenvektor.} Hier bleibt nur \emph{eine} echte Zeile \"ubrig (die anderen zwei sind $0 = 0$), n\"amlich
\[
x + y + z = 0 \;\Rightarrow\; x = -y - z.
\]
Eine Gleichung, drei Unbekannte $\Rightarrow$ diesmal sind \textbf{zwei} Variablen frei: nur $x$ hat einen Pivot, also sind $y$ und $z$ frei. Den Basisvektor zu jeder freien Variable bekommt man, indem man sie auf $1$ setzt und die andere auf $0$ (und $x = -y-z$ ausrechnet):
\begin{itemize}[leftmargin=*, nosep]
  \item $(y, z) = (1, 0)$: $x = -1 - 0 = -1 \Rightarrow (-1, 1, 0)$.
  \item $(y, z) = (0, 1)$: $x = 0 - 1 = -1 \Rightarrow (-1, 0, 1)$.
\end{itemize}
\textbf{Zwei} freie Variablen $\Rightarrow$ \textbf{zwei} Basisrichtungen, also GM $= 2$:
\[
...
\textbf{Faktorisieren.} $\chi_B(t) = -(t^3 - 8t^2 + 20t - 16)$. Teiler von $16$ testen: $t = 2$ und $t = 4$ ergeben $0$. Dritte Nullstelle aus der Spur ($2 + 4 + \lambda_3 = 8$): $\lambda_3 = 2$. Nullstellen: $\mathbf{2,\ 2,\ 4}$. Wie bei $A$ wird jede Nullstelle zu einem Faktor, die doppelte $2$ zum Quadrat:
\[
...
\]
Aus der Stufenform: die $(-1)$-Zeile gibt $-y = 0 \Rightarrow y = 0$; die $\piv{1}$-Zeile gibt $x + y + z = 0 \Rightarrow x = -z$. Frei ist nur $z$ -- also \textbf{nur eine} Basisrichtung, obwohl AM $= 2$ ist (genau das macht $B$ defekt). Mit $z = 1$:
\[
...
\[
A v = 0:\quad \text{Zeile 2:}\;\; 6y = 0 \Rightarrow y = 0, \qquad \text{Zeile 1:}\;\; x + 5z = 0 \Rightarrow x = -5z.
\]
...
\textbf{Schritt 2: $\lambda = 6$.} Eigenraum $= \ker(A - 6I)$. Zuerst
\[
...
\]
Beide nichttriviale Zeilen sagen dasselbe: $x = z$. Auf $y$ gibt es \emph{keine} Bedingung $\Rightarrow$ $y$ und $z$ sind frei (zwei freie Variablen). Mit $(y, z) = (1, 0)$ und $(0, 1)$:
\[
...
\textbf{Schritt 1: $\lambda = 0$ mit GM $= 3$.} Alle Spalten von $B$ sind gleich $\Rightarrow \operatorname{Rang}(B) = 1 \Rightarrow \dim\ker(B) = 4 - 1 = 3$. Also ist $0$ \EW{} mit $\text{GM}(0) = 3$. Der Kern: $Bv = 0$ hei\ss t (jede Zeile) $5(a+b+c+d) = 0$, also $a + b + c + d = 0$. Das ist \emph{eine} Gleichung f\"ur \emph{vier} Unbekannte $\Rightarrow$ \textbf{drei} freie Variablen ($b, c, d$); jeweils eine auf $1$ setzen (Rest $0$, dann $a = -1$) gibt die drei Basisvektoren:
\[
