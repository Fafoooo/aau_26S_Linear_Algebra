...
\[
p(x) = x - 1, \quad q(x) = 2 - x
\]

$p \in W$? $\quad p(1) = 0$, $p(2) = 1$ $\;\Rightarrow\; p(1) \cdot p(2) = 0 \cdot 1 = 0$ \checkmark{} \quad (weil $p(1) = 0$)

$q \in W$? $\quad q(1) = 1$, $q(2) = 0$ $\;\Rightarrow\; q(1) \cdot q(2) = 1 \cdot 0 = 0$ \checkmark{} \quad (weil $q(2) = 0$)

Jetzt addieren -- und es passiert etwas Sch\"ones:
\[
(p + q)(x) = (x - 1) + (2 - x) = 1
\]

Die Summe ist einfach die \textbf{Konstante 1}! Also:
\[
(p+q)(1) = 1, \quad (p+q)(2) = 1
\]
...
\textbf{Warum passiert das?} $p$ ist bei $x=1$ null, $q$ ist bei $x=2$ null. Wenn man sie addiert, heben sich die $x$-Terme auf und es bleibt nur die Konstante $1$ \"ubrig -- die ist \textbf{nirgends} null!
...
\medskip
\textbf{(U1) Nullvektor:}
Die Nullfunktion ist die Funktion $f(x) = 0$ f\"ur \textbf{alle} $x \in \mathbb{R}$.
Ich muss pr\"ufen: Gilt $f(-x) = -f(x)$?
\[
f(-x) = 0 \quad \text{und} \quad -f(x) = -0 = 0.
\]
$0 = 0$ \checkmark \quad Also ist die Nullfunktion ungerade $\Rightarrow$ Nullfunktion $\in W$.

\medskip
\textbf{(U2) Addition:}
Seien $f, g \in W$, d.\,h.\ f\"ur alle $x$ gilt: $f(-x) = -f(x)$ und $g(-x) = -g(x)$.

Ich muss zeigen: $(f+g)(-x) = -(f+g)(x)$ f\"ur alle $x$.

Schritt f\"ur Schritt:
\begin{align*}
(f+g)(-x) &= f(-x) + g(-x)
    && \text{(Definition der Addition von Funktionen)} \\
  &= (-f(x)) + (-g(x))
    && \text{(weil $f$ und $g$ ungerade sind)} \\
  &= -(f(x) + g(x))
    && \text{(Minus ausklammern)} \\
  &= -(f+g)(x). \;\checkmark
\end{align*}

\begin{analogie}
Stell dir vor, $f$ und $g$ sind wie zwei Waagen, die bei negativem Input das Vorzeichen umdrehen. Wenn ich zwei solche Waagen zusammenlege (addiere), dreht die kombinierte Waage immer noch das Vorzeichen um -- das Umdrehen ``\"uberlebt'' die Addition.
\end{analogie}

\textbf{(U3) Skalarmultiplikation:}
Sei $f \in W$ (also $f(-x) = -f(x)$) und $k \in \mathbb{R}$.

Ich muss zeigen: $(kf)(-x) = -(kf)(x)$ f\"ur alle $x$.

Schritt f\"ur Schritt:
\begin{align*}
(kf)(-x) &= k \cdot f(-x)
    && \text{(Definition: $(kf)$ bedeutet ``$k$ mal $f$'')} \\
  &= k \cdot (-f(x))
    && \text{(weil $f$ ungerade ist)} \\
  &= -(k \cdot f(x))
    && \text{(Minus rausziehen)} \\
  &= -(kf)(x). \;\checkmark
\end{align*}
...
\medskip
\textbf{Gegenbeispiel 1 -- Nullvektor (U1) verletzt:}

Die Nullfunktion ist $f(x) = 0$ f\"ur alle $x$.
Die Bedingung verlangt: $f(x) \neq 0$ f\"ur alle $x \neq 0$.

Testen wir z.\,B.\ $x = 1$ (das ist $\neq 0$):
\[
f(1) = 0 \quad \text{-- aber die Bedingung verlangt } f(1) \neq 0. \quad \Lightning
\]

Die Nullfunktion gibt \"uberall $0$ zur\"uck -- also auch an Stellen $x \neq 0$.
Das verletzt die Bedingung. $\Rightarrow$ Nullfunktion $\notin W$.

\begin{merkbox}
Schon damit ist klar: $W$ ist kein Unterraum, denn (U1) ist verletzt.
\end{merkbox}

\textbf{Gegenbeispiel 2 -- Addition (U2) verletzt:}

Selbst wenn wir (U1) ignorieren, scheitert es auch an der Addition:

\begin{itemize}
\item $f(x) = x$: Ist $f \in W$?
    F\"ur $x \neq 0$: $f(x) = x \neq 0$ \checkmark.
    Also ja, $f \in W$.

\item $g(x) = -x$: Ist $g \in W$?
    F\"ur $x \neq 0$: $g(x) = -x \neq 0$ \checkmark.
    Also ja, $g \in W$.

\item Summe: $(f+g)(x) = x + (-x) = 0$ f\"ur \textbf{alle} $x$.
    Die Summe ist die Nullfunktion!

    Testen wir $x = 1$: $(f+g)(1) = 0$, aber $1 \neq 0$.
    Die Bedingung verlangt $(f+g)(1) \neq 0$. \Lightning
\end{itemize}

Zwei Funktionen aus $W$ addiert ($x$ und $-x$) heben sich gegenseitig auf -- die Summe ist \"uberall $0$, und das ist nicht in $W$.
