...
\newcommand{\orient}{\textcolor{gray}{\small\textit{[Orientierung im Viereck -- nicht Teil der L\"osung]}}}
...
% Viereck blass im Hintergrund
\draw[->, black!12] (vE) -- (wE);
\draw[->, black!12] (vB) -- (wB);
\draw[->, black!12] (vE) -- (vB);
\draw[->, black!12] (wB) -- (wE);
% aktiv: die zwei senkrechten Pfeile, die wir jetzt bauen
...
\end{tikzpicture}
\end{center}
{\small\itshape\color{gray} Wir sind hier: Wir bauen nur die beiden \emph{senkrechten} Pfeile $P$ und $P^{-1}$ -- die ``\"Ubersetzer'' zwischen Standard- und $B$-Welt. Die waagrechten Pfeile fassen wir noch nicht an.}
...
\textbf{Schritt 2: $[T]_E = P\,[T]_B\,P^{-1}$ ausrechnen.}
...
\node (vB) at (0,0)     {$[v]_B$};
\node (wB) at (4.5,0)   {$[T(v)]_B$};
% Ziel (oberer Pfeil) blass gestrichelt
\draw[->, blue!35, dashed] (vE) -- (wE) node[midway, above]{$[T]_E$ (Ziel)};
% Umweg, in Rechenreihenfolge durchnummeriert
...
\end{tikzpicture}
\end{center}
{\small\itshape\color{gray} Wir sind hier: Jetzt laufen wir den Umweg im Viereck ab -- quasi im Kreis von links oben nach rechts oben. \textbf{(1)} mit $P^{-1}$ runter in die $B$-Welt, \textbf{(2)} unten die bekannte Matrix $[T]_B$ anwenden, \textbf{(3)} mit $P$ wieder rauf. Dieser ganze Weg ist der gesuchte obere Pfeil. In genau dieser Reihenfolge rechnen wir: erst $[T]_B P^{-1}$ (Pfeile 1+2), dann $P$ davor (Pfeil 3).}
...
\textbf{Schritt 3: Formel ablesen.}
...
\node (vB) at (0,0)     {$[v]_B$};
\node (wB) at (4.5,0)   {$[T(v)]_B$};
% Umweg jetzt blass -- haben wir hinter uns
\draw[->, black!12] (vE) -- (vB);
\draw[->, black!12] (vB) -- (wB);
\draw[->, black!12] (wB) -- (wE);
% der obere Pfeil ist jetzt fertig
\draw[->, blue!70, line width=1.1pt] (vE) -- (wE) node[midway, above]{$[T]_E$ (jetzt bekannt)};
\end{tikzpicture}
\end{center}
{\small\itshape\color{gray} Wir sind am Ziel: Der obere Pfeil ist fertig, $[T]_E = \left(\begin{smallmatrix}1&0\\4&-1\end{smallmatrix}\right)$. Weil in der Standardbasis $[v]_E = v$ ist, liest man die Formel direkt aus dieser Matrix ab.}

Die Matrix $[T]_E$ angewandt auf $(x, y)$:
\[
