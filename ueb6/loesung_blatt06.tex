...
Wir benutzen die erste Zeile als Pivot-Zeile und machen darunter in der ersten Spalte Nullen:
\[
...
Also bringt sie keine neue Information:
\[
...
Jetzt schieben wir die Nullzeile nach unten und machen aus dem Pivot $-1$ eine $1$:
\[
...
Zum Schluss machen wir den Eintrag \"uber dem Pivot in Spalte $z$ zu $0$:
\[
...
Die Pivot-Spalten sind $x$ und $z$.
...
\begin{begriffe}
\begin{itemize}[leftmargin=*, nosep]
    \item Die Zeilen von $A$ sind genau die drei Vektoren, die $U$ aufspannen.
    \item Die Spalten von $A$ sind genau die f\"unf Vektoren, die $W$ aufspannen.
    \item Der L\"osungsraum aus 23(a) ist der Kern von $A$, also $\ker(A)$.
\end{itemize}
\end{begriffe}
