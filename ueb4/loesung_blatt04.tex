...
Pivot-Spalten: $\alpha, \beta$. Spalte ohne Pivot: $\gamma$ $\Rightarrow$ $\gamma$ frei.
...
Alle drei Spalten ($\alpha, \beta, \gamma$) haben ein Pivot, keine freie Variable.
...
\textbf{Schritt 3: Pivots ablesen und Fallunterscheidung.}

Die Kandidaten f\"ur Pivots sind die drei markierten Eintr\"age. Die ersten beiden ($1$ und $-2$) sind immer $\neq 0$. Der dritte, $k-8$, ist nur dann ein \emph{echtes} Pivot, wenn $k-8 \neq 0$.

\begin{itemize}[leftmargin=*, nosep]
\item $k \neq 8$: alle drei Pivots vorhanden $\Rightarrow$ Rang $= 3 =$ Anzahl der Vektoren $\Rightarrow$ linear unabh\"angig.
\item $k = 8$: das dritte Pivot wird $0$, Zeile 3 wird zur Nullzeile $\Rightarrow$ Rang $= 2 < 3 \Rightarrow$ linear abh\"angig.
\end{itemize}
...
\textbf{Schritt 3: Pivots ablesen und auswerten.}

Die ersten beiden Pivots ($1$ und $-2$) sind immer $\neq 0$. Das dritte ist $2(k-8)$, und das wird $0$ genau dann, wenn $k = 8$.

\begin{itemize}[leftmargin=*, nosep]
\item $k \neq 8$: drei Pivots, keine Nullzeile $\Rightarrow$ linear unabh\"angig.
\item $k = 8$: Zeile 3 wird komplett zur Nullzeile $\Rightarrow$ linear abh\"angig.
\end{itemize}
