...
\begin{enumerate}[leftmargin=*, nosep]
    \item Polynome ausmultiplizieren, sodass sie in der Form $ax^2 + bx + c$ dastehen.
    \item Ansatz $\alpha p_1 + \beta p_2 + \gamma p_3 = 0$ aufstellen und ausmultiplizieren.
    \item \textbf{Koeffizientenvergleich}: f\"ur jede Potenz von $x$ (also $x^2$, $x$, $1$) eine eigene Gleichung aufstellen. So wird aus dem Polynom-Problem ein ganz normales lineares Gleichungssystem.
    \item Gleichungssystem mit Gau\ss-Elimination l\"osen (wie gewohnt).
    \item Pivots ablesen und entscheiden:
    \begin{itemize}[nosep]
        \item Nur triviale L\"osung $\Rightarrow$ linear unabh\"angig (Ende).
        \item Nichttriviale L\"osung $\Rightarrow$ linear abh\"angig. Dann noch eine konkrete Kombination $\alpha p_1 + \beta p_2 + \gamma p_3 = 0$ mit nicht allen Koeffizienten null angeben.
    \end{itemize}
    \item Probe: Kombination einsetzen, ausmultiplizieren, muss das Nullpolynom ergeben.
\end{enumerate}
\end{wastun}
...
Ich setze die ausmultiplizierten Polynome aus Schritt 1 in den Ansatz ein:
\begin{align*}
\alpha \cdot \underbrace{(x^2 - x - 2)}_{p_1}
+ \beta \cdot \underbrace{(x^2 - 2x - 3)}_{p_2}
+ \gamma \cdot \underbrace{(x^2 - 3x - 4)}_{p_3} &= 0
\end{align*}

Ausmultiplizieren und nach Potenzen von $x$ sortieren:
\begin{align*}
&\alpha x^2 - \alpha x - 2\alpha + \beta x^2 - 2\beta x - 3\beta + \gamma x^2 - 3\gamma x - 4\gamma = 0 \\
\iff\;& \underbrace{(\alpha + \beta + \gamma)}_{\text{Koeff.\ von } x^2} x^2
+ \underbrace{(-\alpha - 2\beta - 3\gamma)}_{\text{Koeff.\ von } x} x
+ \underbrace{(-2\alpha - 3\beta - 4\gamma)}_{\text{konstanter Term}} = 0
\end{align*}

Die rechte Seite ist das Nullpolynom -- also muss jeder Koeffizient einzeln $0$ sein:
\[
\begin{cases}
\alpha + \beta + \gamma = 0 & \text{(Koeffizient von $x^2$: aus jedem $p_i$ kommt ein $x^2$)} \\
-\alpha - 2\beta - 3\gamma = 0 & \text{(Koeffizient von $x$: aus $p_1, p_2, p_3$ kommen $-1, -2, -3$)} \\
-2\alpha - 3\beta - 4\gamma = 0 & \text{(konstanter Term: aus $p_1, p_2, p_3$ kommen $-2, -3, -4$)}
\end{cases}
\]
...
\textbf{Schritt 5: Konkrete Koeffizienten $\alpha, \beta, \gamma$ ausrechnen.}

Ich brauche jetzt konkrete Zahlen f\"ur $\alpha, \beta, \gamma$, die nicht alle null sind und das Gleichungssystem erf\"ullen.

\textbf{(a) Freie Variable w\"ahlen.} $\gamma$ ist frei $\Rightarrow$ ich darf irgendeine Zahl reinsetzen. Ich nehme $\gamma = 1$ (einfachster Wert $\neq 0$).

\textbf{(b) $\beta$ aus Zeile 2 ausrechnen.} Zeile 2 lautet $-\beta - 2\gamma = 0$. Mit $\gamma = 1$:
\[
-\beta - 2 = 0 \;\Rightarrow\; \beta = -2.
\]

\textbf{(c) $\alpha$ aus Zeile 1 ausrechnen.} Zeile 1 lautet $\alpha + \beta + \gamma = 0$. Mit $\beta = -2$ und $\gamma = 1$:
\[
\alpha - 2 + 1 = 0 \;\Rightarrow\; \alpha = 1.
\]

\textbf{Ergebnis:} $(\alpha, \beta, \gamma) = (1, -2, 1)$ -- nicht alle null, also nichttrivial.
...
\begin{wastun}
\begin{enumerate}[leftmargin=*, nosep]
    \item Ansatz $\alpha \cdot 1 + \beta \cdot 2^x + \gamma \cdot 3^x = 0$ hinschreiben (soll f\"ur alle $x$ gelten).
    \item \textbf{Drei ``einfache'' $x$-Werte einsetzen} (z.\,B.\ $x = 0, 1, 2$). Jede Einsetzung liefert eine gew\"ohnliche Zahlengleichung $\Rightarrow$ drei Gleichungen in drei Unbekannten $\alpha, \beta, \gamma$.
    \item Gau\ss-Elimination durchf\"uhren.
    \item Pivots ablesen und entscheiden:
    \begin{itemize}[nosep]
        \item Drei Pivots $\Rightarrow$ nur die triviale L\"osung $\Rightarrow$ linear \textbf{unabh\"angig}.
        \item Weniger als drei Pivots $\Rightarrow$ nichttriviale L\"osung $\Rightarrow$ linear \textbf{abh\"angig}.
    \end{itemize}
    \item Folgerung: Das Ergebnis an drei Punkten gilt erst recht, wenn man es f\"ur \textbf{alle} $x$ fordert (die Bedingung ``alle $x$'' ist st\"arker).
\end{enumerate}
\end{wastun}
...
\textbf{(a) Ansatz hinschreiben.} Die Bedingung lautet:
\[
\alpha \cdot 1 + \beta \cdot 2^x + \gamma \cdot 3^x = 0 \quad \text{f\"ur \textbf{alle} } x \in \mathbb{R}.
\]

\textbf{(b) Trick: drei konkrete $x$-Werte einsetzen.} Wenn die Gleichung f\"ur jedes $x$ gelten muss, dann auch f\"ur drei konkrete. Ich w\"ahle die einfachsten: $x = 0, 1, 2$.

\textbf{(c) $x = 0$ einsetzen.} Mit $2^0 = 1$ und $3^0 = 1$:
\[
\alpha \cdot 1 + \beta \cdot 1 + \gamma \cdot 1 = 0 \;\Rightarrow\; \alpha + \beta + \gamma = 0.
\]

\textbf{(d) $x = 1$ einsetzen.} Mit $2^1 = 2$ und $3^1 = 3$:
\[
\alpha \cdot 1 + \beta \cdot 2 + \gamma \cdot 3 = 0 \;\Rightarrow\; \alpha + 2\beta + 3\gamma = 0.
\]

\textbf{(e) $x = 2$ einsetzen.} Mit $2^2 = 4$ und $3^2 = 9$:
\[
\alpha \cdot 1 + \beta \cdot 4 + \gamma \cdot 9 = 0 \;\Rightarrow\; \alpha + 4\beta + 9\gamma = 0.
\]
...
\begin{wastun}
Die Aufgabe verlangt \textbf{beide Methoden aus der Vorlesung}. Also zweimal das Ergebnis zeigen -- einmal mit Methode 1, einmal mit Methode 2. Beide m\"ussen dasselbe herauskommen.

\medskip

\textbf{Methode 1} (Homogene Gleichung):
\begin{enumerate}[leftmargin=*, nosep]
    \item Ansatz $\alpha v_1 + \beta v_2 + \gamma v_3 = 0$; eintragsweise in 4 Gleichungen zerlegen (je eine pro Komponente von $\mathbb{R}^4$).
    \item Gau\ss-Elimination, mit $k$ als Parameter mitgeschleppt.
    \item Pivots ablesen. Wo taucht $k$ auf? $\Rightarrow$ dort entsteht die Bedingung f\"ur ``echtes'' Pivot.
    \item Fallunterscheidung: F\"ur welche $k$-Werte werden alle Pivots $\neq 0$?
\end{enumerate}

\textbf{Methode 2} (Vektoren als Zeilen):
\begin{enumerate}[leftmargin=*, nosep]
    \item Die drei Vektoren als Zeilen untereinander schreiben ($3 \times 4$-Matrix).
    \item Gau\ss-Elimination durchf\"uhren.
    \item Pivots bzw.\ Nullzeilen ablesen: Wenn eine Zeile komplett $0$ wird, waren die Vektoren linear abh\"angig.
    \item Gleiche Fallunterscheidung nach $k$.
\end{enumerate}

\textbf{Zusatz:} Im Fall $k = 8$ eine konkrete nichttriviale Linearkombination angeben, die $\vec 0$ ergibt (zur Probe).
\end{wastun}
...
\textbf{Schritt 4: Konkrete Koeffizienten bei $k = 8$ ausrechnen.}

\textbf{(a) $k = 8$ in die Stufenform einsetzen.} Mit $k - 8 = 0$ und $k - 6 = 2$ werden die Zeilen zu:
\begin{align*}
...
\textbf{(b) Freie Variable w\"ahlen.} $\gamma$ ist frei $\Rightarrow$ ich nehme $\gamma = 1$.

\textbf{(c) $\beta$ aus Zeile 2 ausrechnen.} Zeile 2 mit $\gamma = 1$:
\[
-2\beta + 2 \cdot 1 = 0 \;\Rightarrow\; -2\beta = -2 \;\Rightarrow\; \beta = 1.
\]

\textbf{(d) $\alpha$ aus Zeile 1 ausrechnen.} Zeile 1 mit $\beta = 1$ und $\gamma = 1$:
\[
\alpha + 1 + 2 \cdot 1 = 0 \;\Rightarrow\; \alpha + 3 = 0 \;\Rightarrow\; \alpha = -3.
\]

\textbf{(e) Probe.} Einsetzen von $(\alpha, \beta, \gamma) = (-3, 1, 1)$ in die urspr\"ungliche Gleichung:
\[
-3(1,4,3,2) + 1 \cdot (1,2,3,4) + 1 \cdot (2,10,6,2) = (0,0,0,0) \;\checkmark
\]
...
\begin{wastun}
Die Aussage ist eine Behauptung. Zwei M\"oglichkeiten:
\begin{itemize}[leftmargin=*, nosep]
    \item \textbf{Wahr beweisen:} zeigen, dass aus $\alpha(v_1+v_2) + \beta(v_2+v_3) + \gamma(v_2+2v_3) = 0$ zwingend $\alpha = \beta = \gamma = 0$ folgt.
    \item \textbf{Falsch zeigen:} ein konkretes Gegenbeispiel (z.\,B.\ $v_1, v_2, v_3$ im $\mathbb{R}^3$), bei dem die neue Menge linear abh\"angig wird.
\end{itemize}

\medskip
\textbf{Konkrete Schritte zum Beweis:}
\begin{enumerate}[leftmargin=*, nosep]
    \item Ansatz $\alpha(v_1+v_2) + \beta(v_2+v_3) + \gamma(v_2+2v_3) = 0$ hinschreiben.
    \item Ausmultiplizieren und \textbf{nach $v_1, v_2, v_3$ sortieren}. So steht vor jedem Basisvektor genau ein Ausdruck in $\alpha, \beta, \gamma$.
    \item Voraussetzung anwenden: Da $\{v_1, v_2, v_3\}$ linear unabh\"angig ist, muss jeder dieser drei Ausdr\"ucke einzeln $0$ ergeben $\Rightarrow$ ein normales Gleichungssystem in $\alpha, \beta, \gamma$.
    \item System l\"osen. Kommt nur $\alpha = \beta = \gamma = 0$ heraus $\Rightarrow$ Aussage wahr.
\end{enumerate}
\end{wastun}
...
\textbf{Schritt 2: Ausmultiplizieren und nach $v_1, v_2, v_3$ sortieren.}

\textbf{(a) Ausmultiplizieren.} Klammern aufl\"osen:
\[
\alpha v_1 + \alpha v_2 + \beta v_2 + \beta v_3 + \gamma v_2 + 2\gamma v_3 = 0.
\]

\textbf{(b) Sammeln pro Vektor.} Terme mit $v_1$, $v_2$, $v_3$ jeweils zusammenfassen:
\begin{itemize}[leftmargin=*, nosep]
    \item Vor $v_1$: nur $\alpha$.
    \item Vor $v_2$: $\alpha + \beta + \gamma$.
    \item Vor $v_3$: $\beta + 2\gamma$.
\end{itemize}
Das ergibt:
\[
...
eine Linearkombination dieser drei Vektoren ist nur dann null, wenn \textbf{jeder} Koeffizient einzeln
...
\textbf{Schritt 4: Konkrete Koeffizienten ausrechnen.}

\textbf{(a) $\alpha$ aus Gleichung 1.} Direkt ablesbar:
\[
\alpha = 0.
\]

\textbf{(b) $\beta$ in Abh\"angigkeit von $\gamma$ aus Gleichung 2.} Mit $\alpha = 0$:
\[
0 + \beta + \gamma = 0 \;\Rightarrow\; \beta = -\gamma.
\]

\textbf{(c) $\gamma$ aus Gleichung 3.} Mit $\beta = -\gamma$:
\[
(-\gamma) + 2\gamma = 0 \;\Rightarrow\; \gamma = 0.
\]

\textbf{(d) Zur\"ucksubstituieren.} Wegen $\beta = -\gamma = 0$ folgt auch $\beta = 0$.
...
\begin{wastun}
``Notwendigerweise linear abh\"angig'' hei\ss t: linear abh\"angig f\"ur \emph{jede} Wahl von $v_1, v_2, v_3$. Wenn das wahr ist, muss es eine feste, universelle nichttriviale Linearkombination der drei Vektoren geben, die \textbf{ohne Voraussetzungen \"uber $v_1, v_2, v_3$} schon $\vec 0$ ergibt.

\medskip
\textbf{Konkrete Schritte:}
\begin{enumerate}[leftmargin=*, nosep]
    \item Die drei gegebenen Vektoren anschauen: $u_1 = 7v_1 - 11v_2$, $u_2 = 11v_2 - 13v_3$, $u_3 = 7v_1 - 13v_3$.
    \item Einfache Kombinationen ausprobieren, z.\,B.\ $u_1 + u_2$. Ziel: Terme sollen sich wegheben, sodass ein anderer $u_i$ entsteht.
    \item Die gefundene Gleichung ``$u_1 + u_2 = u_3$'' umstellen zu einer Linearkombination der Null: $u_1 + u_2 - u_3 = 0$.
    \item Koeffizienten pr\"ufen: nicht alle null $\Rightarrow$ nichttrivial $\Rightarrow$ linear abh\"angig.
    \item Hinweis darauf, dass die gefundenen Koeffizienten $(1, 1, -1)$ nicht von $v_1, v_2, v_3$ abh\"angen $\Rightarrow$ gilt universell.
\end{enumerate}
\end{wastun}
...
\textbf{(a) Einfache Kombination ausprobieren: Summe der ersten beiden.}
\[
(7v_1 - 11v_2) + (11v_2 - 13v_3)
\]

\textbf{(b) Klammern aufl\"osen und sortieren.}
\[
= 7v_1 - 11v_2 + 11v_2 - 13v_3.
\]

\textbf{(c) Gleiche Terme zusammenfassen.} Die $v_2$-Terme heben sich auf ($-11v_2 + 11v_2 = 0$):
\[
= 7v_1 + 0 - 13v_3 = 7v_1 - 13v_3.
\]

\textbf{(d) Ergebnis vergleichen.} Das ist genau der dritte Vektor der Menge:
\[
...
\textbf{(a) Gleichung umstellen.} Von der Form ``Vektor 1 $+$ Vektor 2 $=$ Vektor 3'' subtrahiere ich Vektor 3 auf beiden Seiten:
\[
(7v_1 - 11v_2) + (11v_2 - 13v_3) - (7v_1 - 13v_3) = 0.
\]

\textbf{(b) Koeffizienten ablesen.} Vor den drei Vektoren steht jeweils:
\[
1 \cdot (7v_1 - 11v_2) + 1 \cdot (11v_2 - 13v_3) + (-1) \cdot (7v_1 - 13v_3) = 0.
\]

Koeffizienten $(\alpha, \beta, \gamma) = (1, 1, -1)$ -- alle ungleich null $\Rightarrow$ \textbf{nichttriviale} Kombination.
